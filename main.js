...
function cata (){
	...
	if (zero==mycat.length-1) {zero=0;catimage.src=mycat[zero];}
			...
			if(zero<=6){if (imageLeft>=document.body.clientWidth) {imageLeft=0;catimagebox.style.paddingLeft=0+"px";}imageLeft+=Math.random()*40;catimagebox.style.paddingLeft=imageLeft+"px";}//随机数控制距离和margin控制方向,判断是否超过界限并重置
